(function () {
    ...
    function _HookedDate() {
        ...
        // new Date(value) / new Date(y,m,d,...) → 透传
    ...
    function _hookedDateNow() {
        ...
        return _lastVirtualTime + (realNow - _lastRealTime) * (1 / _percentage);
    }
    ...
    function _applySpeed(speed) {
        ...
        // 更新虚拟时间锚点
        var realNow = _origDateNow();
        _lastVirtualTime = _lastVirtualTime + (realNow - _lastRealTime) * (1 / _percentage);
        _lastRealTime = realNow;
    ...
    function _mountUI() {
        ...
        function _showError(msg) {
            ...
            _origSetTimeout.call(window, function () { _errorToast.style.display = 'none'; }, 2000);
        }
        ...
        function _closePanel() {
            ...
            _origSetTimeout.call(window, function () { _panel.style.display = 'none'; }, 200);
        }

        function _showStatus(speed) {
            _statusIndicator.textContent = speed !== 1 ? '加速已启用: ' + speed + 'x' : '速度已重置';
            _statusIndicator.style.display = 'block';
            _origSetTimeout.call(window, function () { _statusIndicator.style.display = 'none'; }, speed !== 1 ? 2000 : 1000);
        }
