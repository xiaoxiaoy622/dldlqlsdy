(function () {
    ...
    var _timeoutIds = {};
    var _autoUniqueId = 1;
    var _lastDatetime = _Date.now();
    var _lastMDatetime = _Date.now();
    ...
    function HookedDate() {
        ...
        if (arguments.length > 1) {
            Object.defineProperty(this, '_innerDate', {
                configurable: false, enumerable: false, writable: false,
                value: new _Date(arguments[0], arguments[1], arguments[2] || 1,
    ...
    var _cssText = [
        ...
        '.moon-view .speed-btn{height:36px;border-radius:6px;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.12);color:#d0d8ff;font-weight:700;font-size:12px;display:flex;align-items:center;justify-content:center;cursor:pointer;padding:0 2px;user-select:none}',
        ...
        function _setSpeedUI(v) {
            ...
            try {
                if (window.$hookTimer && typeof window.$hookTimer.setSpeed === 'function') {
                    ...
                    _showError('未找到 $hookTimer，加速可能无效');
                    _showStatus(v);
                }
            } catch (error) {
